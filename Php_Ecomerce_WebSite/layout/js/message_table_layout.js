function initializemessagetable() {
    ...
    function loadPage(page) {
        ...
        xhr.send('table=message&page=' + page);
    }
    // function to update table content
function open_delete(productId){
        var mod = document.getElementById("delete_message_modal");
        var btn=  document.getElementById("message_id");
        btn.setAttribute('value',productId);
        showModal(mod);
}
function open_message(message_client, address_client) {
    var mod = document.getElementById("message_content");
    var area = document.getElementById("mess");
    var btnRepond = document.getElementById("corriel");
    area.innerHTML = message_client;
    btnRepond.innerHTML = '<a  href="mailto:' + address_client + '?subject=Répondre au client">Répondre</a>';
...
///////: close message
var closemess = document.getElementById("close_mess");
closemess.addEventListener("click", function () {
    var mod = document.getElementById("message_content");
    closeModal(mod);
});
function updateTable(productsData) {
    var tableBody = document.getElementById("table-message");
    tableBody.innerHTML = ''; // Clear existing table content
    productsData.forEach(function(message, index) {
        ...
        cell1.textContent = message.nom;
        row.appendChild(cell1);
        ////////////////////////////////////////
        var cell2 = document.createElement('td');
        cell2.scope = 'row';
        cell2.className = 'px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white';
        cell2.textContent = message.teliphone;
        row.appendChild(cell2);
        //////////////////////////////////////
        var cell3 = document.createElement('td');
        cell3.className = ' px-6 py-4';
        cell3.textContent = message.email;
        row.appendChild(cell3);
        ///////////////////////////
        var cell4 = document.createElement('td');
        cell4.className = ' pl-[5%] pr-[5%]';
        var showButton = document.createElement('button');
        //deleteButton.className = 'open-delete_modal';
        showButton.innerHTML = '<i style="font-size: 26px; color: red;" class="ri-delete-bin-6-line"></i>';
        showButton.onclick = function(productId) {
                return function() {
        ...
        cell.className = ' pl-[5%] pr-[5%]';
        var showmessage = document.createElement('button');
        //deleteButton.className = 'open-delete_modal';
        showmessage.innerHTML = '<i style="font-size: 26px; color: blue;"  class="ri-message-2-fill"></i>';
        showmessage.onclick = function(mess,adress) {
                return function() {
                    // Call the open_delete function with the productId
                    open_message(mess,adress);
                };
        }(message.messag,message.email);
        cell.appendChild(showmessage);
...
function updatePagination(currentPage, totalPages) {
    var paginationContainer = document.getElementById('pagination_message-links');
...
document.getElementById('delete_message_forme').addEventListener('submit', function(event) {
                ...
                if (data.success) {
                    ...
                    Swal.fire({
                        ...
                        onClose: function() {
                            ...
                            var modal = document.getElementById("delete_message_modal");
                            closeModal(modal);
                            loadPage(1);
                        }
                    });
